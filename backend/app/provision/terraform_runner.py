...
def write_tfvars(workspace_dir: str, config: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        ...
        f'disk_size_gb  = {int(config.get("disk_size_gb") or 30)}',
        f'ami_id        = "{config.get("ami_id", "")}"',
        f'ec2_os        = "{config.get("ec2_os", "amazon_linux_2")}"',
        f'ec2_user_data = {json.dumps(config.get("ec2_user_data") or "")}',
        f'bucket_name   = "{bucket_name}"',
        f'dynamodb_table_name = "{config.get("dynamodb_table_name", "")}"',
        f'role_name     = "{config.get("role_name", "app-role")}"',
        f'iam_role_preset = "{config.get("iam_role_preset", "s3_read_only")}"',
...
def _write_gcp_tfvars(workspace_dir: str, config: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f'enable_gcp_network         = {str(config.get("enable_gcp_network", False)).lower()}',
        f'enable_gce                 = {str(config.get("enable_gce", False)).lower()}',
        f'enable_gcp_service_account = {str(config.get("enable_gcp_service_account", False)).lower()}',
        f'enable_gcp_monitoring      = {str(config.get("enable_gcp_monitoring", False)).lower()}',
        f'enable_firestore            = {str(config.get("enable_firestore", False)).lower()}',
        "",
        f'bucket_name            = "{bucket_name}"',
        f'instance_name          = "{config.get("instance_name", "main-instance")}"',
        f'machine_type           = "{config.get("machine_type", "e2-micro")}"',
        f'disk_size_gb           = {int(config.get("disk_size_gb") or 30)}',
        f'service_account_id     = "{config.get("service_account_id", "zenith-app-sa")}"',
        f'firestore_database_id  = "{config.get("firestore_database_id", "(default)")}"',
        f'alarm_email            = "{config.get("alarm_email", "")}"',
        "",
        "tags = {",
    ]
...
def _write_azure_tfvars(workspace_dir: str, config: dict[str, Any]) -> str:
    lines = [
        ...
        f'enable_vnet          = {str(config.get("enable_vnet", False)).lower()}',
        f'enable_azure_vm      = {str(config.get("enable_azure_vm", False)).lower()}',
        f'enable_azure_monitor = {str(config.get("enable_azure_monitor", False)).lower()}',
        f'enable_cosmos        = {str(config.get("enable_cosmos", False)).lower()}',
        "",
        f'instance_name        = "{config.get("instance_name", "main-instance")}"',
        f'vm_size              = "{config.get("vm_size", "Standard_B1s")}"',
        f'disk_size_gb         = {int(config.get("disk_size_gb") or 30)}',
        f'cosmos_account_name  = "{config.get("cosmos_account_name", "")}"',
        f'cosmos_database_name = "{config.get("cosmos_database_name", "zenith-db")}"',
        f'alarm_email          = "{config.get("alarm_email", "")}"',
        "",
        "tags = {",
    ]
